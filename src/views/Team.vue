        ...
        <el-table :data="Members" style="width: 100%">
          ...
          <el-table-column prop="id" label="操作">
            <template slot-scope="scope">
              ...
              <el-button v-if="scope.row.status==='管理员'&&myStatus==='团队发起者'" type="warning"
                         @click="delAdmin(scope.row.email)">移除管理员
              </el-button>
              <el-button v-if="scope.row.status==='管理员'&&myStatus==='团队发起者'" type="danger"
                         @click="out(scope.row.email)">移出团队
              </el-button>
              <el-button v-if="scope.row.status==='普通成员'&&myStatus!=='普通成员'" type="primary"
                         @click="addAdmin(scope.row.email)">设为管理员
              </el-button>
              <el-button v-if="scope.row.status==='普通成员'&&myStatus!=='普通成员'" type="danger"
                         @click="out(scope.row.email)">移出团队
              </el-button>
            </template>
          </el-table-column>
